...
ggsave("Workflow/3_Meta-transcriptomics_analysis/Outputs/Figures/sc_PCA.png", 
       gg.pca_sc, width = 7.5, height = 6, dpi = 300)
...
ko_lt <- ko_df.f[,c("KEGG_ko", sample_lt$Sample_ID)]

dds_lt <- DESeqDataSetFromMatrix(countData = ko_lt, 
                                 colData = sample_lt, 
                                 design = ~ Condition + Origin, tidy = TRUE)

keep <- rowSums(counts(dds_lt)) >= 10
dds_lt <- dds_lt[keep,]
...
gg.pca_lt <- ggplot(pcaData_lt, aes(PC1, PC2, color = Condition)) +
  geom_point(size = 4) + 
